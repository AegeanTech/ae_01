	...
	<head>
		...
		<script>
			jQuery(document).ready(function(){
				pegasusGoogleAutocomplete_wResults("#dicaddress", function(response){
					if(jQuery('#google_state').length>0){
						jQuery('#google_state').val(response.state_code);
					}

					if(jQuery('#google_city').length>0){
						jQuery('#google_city').val(response.city_code);
					}

					if(jQuery('#diccountry').length>0){
						jQuery('#diccountry').val(response.country_code).trigger("change");
					}else if(jQuery('#dicnom').length>0){
						jQuery('#dicnom').val(response.state_code).trigger("change");
					}

					if(jQuery('#p7').length>0){
						jQuery('#p7').val(response.postcode);
					}

					if(jQuery('#p5').length>0){
					  jQuery('#p5').val(response.area);
					}
					
					jQuery('#address_conf').val(response.transaddress);
					jQuery('#geoloclat').val(response.geoloclat);
					jQuery('#geoloclng').val(response.geoloclng);
				});
			});

			function pegasusGoogleAutocomplete_wResults(fld, callback){
				jQuery(fld).autocomplete({
					noCache: true,
					minChars: 2,
					serviceUrl: 'makeAddress.php',
					onSelect: function (suggestion) {
						jQuery(this).val(suggestion.transaddress);
						jQuery.ajax({
							url: "find_geovals.php",
							data: suggestion,
							method: "POST",
							dataType: "json",
