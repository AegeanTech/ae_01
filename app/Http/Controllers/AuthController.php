...
class AuthController extends JoshController
{
        ...
        try {
            ...
            if(Sentinel::authenticate(Input::only('email', 'password'), Input::get('remember-me', false)))
            {
                ...
                foreach ($umessages as $umessage){
                    if (($umessage->status == 'active') && ($umessage->state == 'unread')){
                        switch ($umessage->type) {
                            case 'danger':
                                $danger_umessage_id .= $umessage->id . ',';
                                $danger_umessage .= '<p>' . $umessage->description . '</p>';
                            break;
                            case 'warning':
                                $warning_umessage_id .= $umessage->id . ',';
                                $warning_umessage = $umessage->description;
                            break;
                            case 'success':
                                $success_umessage_id .= $umessage->id . ',';
                                $success_umessage .= $umessage->description . '<br>';
                            break;
                            case 'info':
                                $info_umessage_id .= $umessage->id . ',';
                                $info_umessage .= '<p>' . $umessage->description . '</p>';
                            break;
                        }
                    }
                }

                if ((isset($danger_umessage)) && ($danger_umessage != '')){
                    \Session::flash('alert-danger', $danger_umessage);
                    \Session::flash('danger_umessage_id', rtrim($danger_umessage_id, ","));
                }
                if ((isset($warning_umessage)) && ($warning_umessage != '')){
                    \Session::flash('alert-warning', $warning_umessage);
                    \Session::flash('warning_umessage_id', rtrim($warning_umessage_id, ","));
                }
                if ((isset($success_umessage)) && ($success_umessage != '')){
                    \Session::flash('alert-success', $success_umessage);
                    \Session::flash('success_umessage_id', rtrim($success_umessage_id, ","));
                }
                if ((isset($info_umessage)) && ($info_umessage != '')){
                    \Session::flash('alert-info', $info_umessage);
                    \Session::flash('info_umessage_id', rtrim($info_umessage_id, ","));
                }
    ...
    public function getLogout()
    {
        ...
        return Redirect::to('admin/signin')->with('success', 'Αποσυνδεθήκατε με επιτυχία!');
    }
